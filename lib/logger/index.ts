...
class Logger {
   ...
   * Log de erro fatal
   */
  async fatal(message: string, error?: Error | unknown, context?: LogContext): Promise<void> {
    ...
    // Em produção, notificar administradores
    if (this.isProduction) {
      try {
        // Enviar notificação urgente para administradores via webhook
  ...
  withContext(context: LogContext): Logger {
    ...
    originalMethods.forEach(method => {
      const original = this[method as keyof Logger] as (...args: unknown[]) => unknown;
