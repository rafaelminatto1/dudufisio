...
class ClinicalReportGenerator {
  ...
  async generatePDF(
    ...
    try {
      // Importar puppeteer dinamicamente para evitar problemas de build
      const puppeteer = await import('puppeteer');
      
      const htmlContent = await this.buildHtmlReport(reportData, this.getTemplate(reportType), options)
